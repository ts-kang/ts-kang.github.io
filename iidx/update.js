/**
 * IIDX CSV Generator r190204, modified version for IIDX 27
 * by @lazykuna
 *
 * MIT License
 *
  ...
  self.leggens =
{
  ...
  "CONTRACT": "CONTRACT†",
  "waxing and wanding": "waxing and wanding†",
  "KAMAITACHI": "KAMAITACHI†",
  "VANESSA": "VANESSA†",
  "Blue Rain": "Blue Rain†",
  "ICARUS": "ICARUS†",
  "THE DEEP STRIKER": "THE DEEP STRIKER†",
  "Ubertreffen": "Ubertreffen†",
  ...
  "Ancient Scapes": "Ancient Scapes†LEGGENDARIA",
  "Feel The Beat": "Feel The Beat†LEGGENDARIA",
  "invoker": "invoker†LEGGENDARIA",
  "KAISER PHOENIX": "KAISER PHOENIX†",
  "Sigmund": "Sigmund†LEGGENDARIA",
  "Verflucht": "Verflucht†LEGGENDARIA",
  "廿": "廿†",
  "疾風迅雷": "疾風迅雷†LEGGENDARIA",
  ...
  function csv_allsong()
  {
    ...
    function _cache_level(p, dom)
    {
	  ...
	  console.log(title + "_" + diff + "_" + style);
      }
    ...
    function _process_series(p, dom)
    {
      ...
      // song scan
    var idx_leggen = 0;
    /* index starts from 1 to exclude begginer difficulty */
      for (var i=1; i<dom_tables.length; i++)
      {
        ...
        for (var j=0; j<dom_tr.length; j++)
        {
          ...
          if (i == 4) {
            if (ex > 0 && clear_type !== "NO PLAY") {
              ++idx_leggen;
              song_cached.splice(j + idx_leggen, 0, _create_leggendaria_row(version, title, r));
            }
          } else if (song_cached.length > j) {
  ...
  function _create_leggendaria_row(version, title, leggen_data)
  {
    var leggen_row = [
      version, /* version */
      leggens[title] || title.concat('†'), /* title */
      "---", /* genre */
      "---", /* artist */
      0, /* play count */
      "",0,0,0,"---","NO PLAY","---","",0,0,0,"---","NO PLAY","---" /* dummy fields */
    ].concat(leggen_data);
